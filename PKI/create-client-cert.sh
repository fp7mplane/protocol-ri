...
sed -i 's/commonName.*/commonName              =  \"'$DN'\"/g' etc/client.conf 
sed -i 's/SAN                     = DNS:.*/SAN                     = DNS:'$DN'.POLITO.monroe.org/g' etc/client.conf 



echo "Creating TLS certificate request....."
openssl req -new -config $MPLANE_DIR/etc/client.conf -out $MPLANE_PKI_DIR/certs/${name}.csr -keyout $MPLANE_PKI_DIR/certs/${name}.key  -passin pass:$PASS -passout pass:$PASS
