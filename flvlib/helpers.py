...
class OrderedAttrDict(DictMixin):
    ...
    # as advised by http://stackoverflow.com/questions/19344046 and 11165188
    def __iter__(self):
         for k in self.keys():
            yield k
    def __len__(self):
    ...
    def __eq__(self, other):
        try:
            my_iter = iter(self.items())
            his_iter = iter(other.items())
        ...
        while True:
            try:
                my_key, my_val = next(my_iter)
            except StopIteration:
                my_empty = True
            try:
                his_key, his_val = next(his_iter)
            except StopIteration:
                his_empty = True
