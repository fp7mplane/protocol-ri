...
class ScriptTag(Tag):
    ...
    def parse_tag_content(self):
        ...
        if strict_parser():
            ...
            log.debug("max offset is 0x%08X (current offset: 0x%08X size: %u)", tag_end, self.offset, self.size)
...
class FLV(object):
    ...
    def parse_header(self):
        ...
        self.header_size = get_ui32(f)
        log.debug("Header size is %d bytes", self.header_size)

        # 9 bytes read so far, skip the rest of the header	
        f.seek(self.header_size - 9, os.SEEK_CUR)
...
def create_flv_tag(type, data, timestamp=0):
    ...
    ret = b''.join([tag_type, make_ui24(data_size), timestamp, stream_id, data, make_ui32(tag_size)])
    log.debug('FLV tag: %r', ret)
    return ret


def create_script_tag(name, data, timestamp=0):
    payload = make_ui8(2) + make_script_data_variable(name, data)
    log.debug("Creating a script tag with name %s and data %r", name, data)
    ret = create_flv_tag(TAG_TYPE_SCRIPT, payload, timestamp)
    # log.debug("Creating a script tag with payload: %s", ":".join("{:02x}".format(c) for c in payload))
    return ret
...
def create_flv_header(has_audio=True, has_video=True):
    ...
    return b''.join(['FLV'.encode('UTF-8'), make_ui8(1), make_ui8(type_flags), make_ui32(9), make_ui32(0)])
