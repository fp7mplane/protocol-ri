...
def make_ecma_array(d):
    ...
    rest = b''.join([make_script_data_variable(name, value)
...
def make_strict_array(l):
    ret = make_ui32(len(l))
    rest = b''.join([make_script_data_value(value) for value in l])
    return ret + rest
...
def get_object(f, max_offset=None):
    ...
    while True:
        ...
        setattr(ret, name.decode('UTF-8'), value)
    return ret
...
def make_object(obj):
    ...
    try:
        ...
        iterator = iter(obj.__dict__.items())
    ret = b''.join([make_script_data_variable(name, value)
