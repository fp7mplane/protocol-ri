...
class ClientShell(cmd.Cmd):
	...
	def __init__(self, config):
		...
		self._client._history = mplane.svgui_handlers.History()
		self._client._reguri = registry_uri
		self._supervisor = ClientGui(config=config, cs=self)
		# self.run()
	...
	def handle_message(self, msg, identity):
		if isinstance(msg, mplane.model.Capability):
			...
			# not yet implemented
			print("handle_message: Withdrawal -- not yet implemented")
			pass
			# raise NotImplementedError("handle_message: Withdrawal handling not implemented yet.")
	...
	def do_runcap(self, arg):
		...
		while self._when is None or \
			...
			try:
				...
				print("Invalid temporal scope "+input)
				return
		...
		for pname in sorted(cap.parameter_names()):
			...
			# self._client._history.add( pname, params[pname] )

		# Now invoke it
		self._client.invoke_capability(cap.get_token(), self._when, params, relabel)
		# print(">>> cap.params = " + str(cap.params))
		mplane.svgui_handlers.addtohistory( self._client._history, self._when, params )
	...
	def do_listmeas(self, arg):
		...
		for label in self._client.result_labels():
			...
			if not isinstance(res, mplane.model.Exception):
				...
				print("Exception: %s (token %s): -- %s" % ( label, res.get_token(), res.__repr__() ))

		# check for duplicates - by tokens and if label than print them as well
		for token in self._client.result_tokens():
...
class ClientGui(mplane.client.BaseClient):
	...
	def __init__(self, config, tls_state=None,
				 supervisor=False, exporter=None, io_loop=None, cs=None):
		super().__init__(tls_state, supervisor=supervisor,
						exporter=exporter)

		self._cs = cs
		self._client = cs._client
		...
		guicfgfile = GUI_CONFIG
		if "gui-config" in config["gui"]:
			guicfgfile = config["gui"]["gui-config"]

		cs._client._guiregfile = GUI_REGISTRY
		if "gui-registry" in config["gui"]:
			client._guiregfile = config["gui"]["gui-registry"]

		guiusrdir = GUI_USERSETTINGSDIR
		if "gui-usrdir" in config["gui"]:
			guiusrdir = config["gui"]["gui-usrdir"]
		...
		self._tornado_application = tornado.web.Application([
			...
			(r"/" + GUI_LOGIN_PATH, mplane.svgui_handlers.LoginHandler, {'supervisor': self._client, 'guicfgfile': guicfgfile }),
			(r"/" + GUI_USERSETTINGS_PATH, mplane.svgui_handlers.UserSettingsHandler, { 'supervisor': self._client, 'guiusrdir': guiusrdir }),
			...
			(r"/" + GUI_PROBES_PATH, mplane.svgui_handlers.ListProbesHandler, { 'supervisor': self._client, 'tlsState': self._tls_state }),
			(r"/" + GUI_STOPPENDINGS_PATH, mplane.svgui_handlers.StopPendingHandler, { 'supervisor': self._client, 'tlsState': self._tls_state }),
			(r"/" + GUI_PORTLETS_PATH, mplane.svgui_handlers.PortletsHandler, { 'supervisor': self._client, 'tlsState': self._tls_state }),
