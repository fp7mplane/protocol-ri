...
integrated tools are:
    -  dealias
    -  neighbourdisc
    -  ping: singleton, aggreggate
    -  trace: standard
    -  tracelb
    -  sniff
    -  tracebox: standard, specific, specific+icmp-quotesize
...
class ScamperTraceboxService(mplane.scheduler.Service):
    ...
    def __init__(self, cap):
        ...
        super(ScamperTraceboxService, self).__init__(cap)
...
class ScamperPingService(mplane.scheduler.Service):
    def __init__(self, cap):
        # verify the capability is acceptable
        if not ((cap.has_parameter("source.ip4") or 
                 cap.has_parameter("source.ip6")) and
            ...
            raise ValueError("capability not acceptable")
        super(ScamperPingService, self).__init__(cap)
...
def trace4_standard_capability(ipaddr):
    ...
    cap.add_parameter("scamper.trace.gaplimit",-1)
    cap.add_parameter("scamper.trace.gapaction",-1)
    cap.add_parameter("scamper.trace.maxttl",-1)
...
class ScamperTraceService(mplane.scheduler.Service):
    ...
    def __init__(self, cap):
        ...
        if not ((cap.has_parameter("source.ip4") or 
                ...
                (cap.has_parameter("destination.ip4") or 
                 cap.has_parameter("destination.ip6"))):
            raise ValueError("capability not acceptable")
        super(ScamperTraceService, self).__init__(cap)
    ...
    def run(self, spec, check_interrupt):
        ...
        # read and parse output from tracebox
        trace_output = []
        for line in trace_process.stdout:
            trace_output.append(line.decode("utf-8"))     
 
        trace_parsed_output = _parse_trace(trace_output)
        ...
        for i, onehop in enumerate(trace_parsed_output):
...
if __name__ == "__main__":
    ...
    if ip4addr is not None:
        scheduler.add_service(ScamperTraceboxService(tracebox4_standard_capability(ip4addr)))
        scheduler.add_service(ScamperTraceboxService(tracebox4_specific_capability(ip4addr)))
        scheduler.add_service(ScamperTraceboxService(tracebox4_specific_quotesize_capability(ip4addr)))
        scheduler.add_service(ScamperPingService(ping4_aggregate_capability(ip4addr)))
        scheduler.add_service(ScamperPingService(ping4_singleton_capability(ip4addr)))
        scheduler.add_service(ScamperTraceService(trace4_standard_capability(ip4addr)))

    if ip6addr is not None:
        scheduler.add_service(ScamperPingService(ping6_aggregate_capability(ip6addr)))
        scheduler.add_service(ScamperPingService(ping6_singleton_capability(ip6addr)))
        scheduler.add_service(ScamperTraceboxService(tracebox6_standard_capability(ip6addr)))
        scheduler.add_service(ScamperTraceboxService(tracebox6_specific_capability(ip6addr)))
        scheduler.add_service(ScamperTraceboxService(tracebox6_specific_quotesize_capability(ip6addr)))    
        scheduler.add_service(ScamperTraceService(trace6_standard_capability(ip6addr)))
