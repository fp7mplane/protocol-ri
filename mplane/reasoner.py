...
class ClientShell(cmd.Cmd):
    ...
    intro = 'mPlane client shell (rev 4.3.2016, monroe branch)\n'\
    ...
    def do_getcap(self, arg):
        ...
        if self._workflow == "client-initiated":
            ...
            self._client.retrieved_token = []

            self._client.retrieve_capabilities(url)

            self.update_capabilities (self._client.get_capabilities_token ())
    ...
    def do_listcap(self, arg):
        ...
        for token in sorted(self._client.capability_tokens()):
            ...
            if cap.get_label() is None:
                ...
                print("Capability %s (token %s)" % (cap.get_label(), cap.get_token()))
    ...
    def do_runcap(self, arg, default_when = None, default_param = None):
        """
        ...
        if (default_when is None):
            ...
            self._when = mplane.model.When(default_when)
            ...
            while pname not in self._defaults or \
                ...
                if single_val is not None:
                    ...
                    if (default_param is None): 
                        ...
                        self._defaults[pname] = default_param[pname]
    ...
    def update_capabilities(self, updated_tokens):
        for token in sorted(self._client.capability_tokens()):
            if token not in updated_tokens and  (token  in self._rrd_token_list):

                """
                cap = self._client.capability_for(token)
                spec = mplane.model.Specification(capability=cap)
                print (spec.get_label())  
                print (self._client._receipt_labels)
                if spec.get_label() in self._client._receipt_labels:
                    rec = self._client.result_for(spec.get_label())
                    if isinstance(rec, mplane.model.Receipt):
                        self.do_stopmeas(spec.get_label() )
                        print("Receipt (token %s): %s" % (spec.get_label(), rec.when()))

                """

                self._client._remove_capability(token)

                if token in self._rrd_token_list: 
                    self._rrd_token_list.remove(token)




    def run_all_rrd_caps(self):
        """
        Run available RRD  export capabilities by label tstat-exporter_rrd 

        """

        # Run RRD exporter
        params = {'repository.url': '130.192.181.142:9000'}

        for token in sorted(self._client.capability_tokens()):
            cap = self._client.capability_for(token)

            if ('tstat-exporter_rrd' in cap.get_label()) and  (token not in self._rrd_token_list) :
                print("Run capability %s with the token = %s" % (cap.get_label(), cap.get_token()))

                self.do_runcap(arg=token, default_when='now + inf', default_param=params)
                self._rrd_token_list.append(token)



def rrd_export_controller (client_shell):

    while True:

        client_shell.run_all_rrd_caps()
        sleep(3600)
        client_shell.do_getcap(arg='130.192.181.137:8890')
