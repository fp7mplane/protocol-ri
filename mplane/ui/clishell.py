...
#               Author: Brian Trammell <brian@trammell.ch>, Arpad Bakay
#
...
class ClientShell(cmd.Cmd):
    ...
    def __init__(self, config, client):
        ...
        self._workflow = "component-initiated"
        if isinstance(self._client, mplane.client.HttpInitiatorClient):
            self._workflow = "client-initiated"
        if "client" in config and self._workflow == "client-initiated":
    ...
    def do_runcap(self, arg):
        ...
        # Now invoke it
        print("Invoking %s with parameters %s" % (cap.get_label(),params));
    ...
    def complete_weblogs(self, text, line, start_index, end_index):
        matches = []
        beginning = line[len("weblogs "):]
        matches.append("ON"[len(beginning)-len(text):])
        matches.append("OFF"[len(beginning)-len(text):])
        return matches

    def do_weblogs(self, arg):
        tornado.log.access_log.setLevel(logging.INFO if arg.upper() == "ON" else logging.WARN) 

    def do_resetclient(self, arg):
        self._client.reset(arg)
