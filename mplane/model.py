...
class When(object):
    ...
    def __str__(self):
        ...
        if self._inner_duration is not None:
            valstr = "".join((valstr, INNER_WHEN_SEP_START, TIME_NOW, RANGE_SEP, unparse_dur(self._inner_duration)))

            if self._inner_period is not None:
                valstr = "".join((valstr, PERIOD_SEP, unparse_time(self._inner_period)))
    ...
    def is_singleton(self):
        ...
        """
        return self._a is not None and self._b is None and self._duration is None
    ...
    def timer_delays(self, tzero=None):
        ...
        if self._b is not None and self._b is not time_future:
            ...
            ed = sd + self._duration.total_seconds()
        else:
            ed = None
    ...
    def datetime_iterator(self, t=None):
        ...
        if not self._repeated:
            raise Exception("Can't get datetime_iterator for non-repeated when") # TODO raise exception or return something here?
        ...
        if self._crontab:
            while True:
                t += period
                if not self.in_scope(t):
                    break
                if len(self._seconds) and (t.second not in self._crontab._seconds):
                    continue
                if len(self._minutes) and (t.minute not in self._crontab._minutes):
                    continue
                if len(self._hours) and (t.hour not in self._crontab._hours):
                    continue
                if len(self._days) and (t.day not in self._crontab._days):
                    continue
                if len(self._weekdays) and ((t.weekday()+1)%7 not in self._crontab._weekdays):
                    continue
                if len(self._months) and (t.month not in self._crontab._months):
                    continue
                yield t
        # repeat without when
        else:
